...
export const CalcAngles = (props) => {
  ...
  function CalculateAllAngles(landmarks) {
    ...
    // shoulders
    let leftShoulderAngle = CalculateAngle(
      landmarks[13],
      landmarks[11],
      landmarks[23]
    );
    let rightShoulderAngle = CalculateAngle(
      landmarks[14],
      landmarks[12],
      landmarks[24]
    );
    ...
    // feet
    let leftFootAngle = CalculateAngle(
      landmarks[25],
      landmarks[27],
      landmarks[31]
    );
    let rightFootAngle = CalculateAngle(
      landmarks[26],
      landmarks[28],
      landmarks[32]
    );
    ...
    let leftArmAngleToGroundMiddlePoint = {
      x: landmarks[11].x,
      y: 10,
      z: landmarks[11].z,
    };
    let leftArmAngleToGround = CalculateAngle(
      leftArmAngleToGroundMiddlePoint,
      landmarks[11],
      landmarks[13]
    );
    let rightArmAngleToGroundMiddlePoint = {
      x: landmarks[12].x,
      y: 10,
      z: landmarks[12].z,
    };
    let rightArmAngleToGround = CalculateAngle(
      rightArmAngleToGroundMiddlePoint,
      landmarks[12],
      landmarks[14]
    );
    // lower arms to ground
    let leftForearmAngleToGroundMiddlePoint = {
      x: landmarks[13].x,
      y: 10,
      z: landmarks[13].z,
    };
    let leftForearmAngleToGround = CalculateAngle(
      leftForearmAngleToGroundMiddlePoint,
      landmarks[13],
      landmarks[15]
    );
    let rightForearmAngleToGroundMiddlePoint = {
      x: landmarks[14].x,
      y: 10,
      z: landmarks[14].z,
    };
    let rightForearmAngleToGround = CalculateAngle(
      rightForearmAngleToGroundMiddlePoint,
      landmarks[14],
      landmarks[16]
    );
    // upper legs to ground
    let leftLegAngleToGroundMiddlePoint = {
      x: landmarks[23].x,
      y: 10,
      z: landmarks[23].z,
    };
    let leftLegAngleToGround = CalculateAngle(
      leftLegAngleToGroundMiddlePoint,
      landmarks[23],
      landmarks[25]
    );
    let rightLegAngleToGroundMiddlePoint = {
      x: landmarks[24].x,
      y: 10,
      z: landmarks[24].z,
    };
    let rightLegAngleToGround = CalculateAngle(
      rightLegAngleToGroundMiddlePoint,
      landmarks[24],
      landmarks[26]
    );
    // lower legs to ground
    let leftShinAngleToGroundMiddlePoint = {
      x: landmarks[25].x,
      y: 10,
      z: landmarks[25].z,
    };
    let leftShinAngleToGround = CalculateAngle(
      leftShinAngleToGroundMiddlePoint,
      landmarks[25],
      landmarks[27]
    );
    let rightShinAngleToGroundMiddlePoint = {
      x: landmarks[26].x,
      y: 10,
      z: landmarks[26].z,
    };
    let rightShinAngleToGround = CalculateAngle(
      rightShinAngleToGroundMiddlePoint,
      landmarks[26],
      landmarks[28]
    );
    // upper body to ground (shoulders to hips angle in relation to the ground)
    let leftShoulderToHipsAngleToGroundMiddlePoint = {
      x: landmarks[23].x,
      y: 10,
      z: landmarks[23].z,
    };
    let leftShoulderToHipsAngleToGround = CalculateAngle(
      leftShoulderToHipsAngleToGroundMiddlePoint,
      landmarks[23],
      landmarks[11]
    );
    let rightShoulderToHipsAngleToGroundMiddlePoint = {
      x: landmarks[24].x,
      y: 10,
      z: landmarks[24].z,
    };
    let rightShoulderToHipsAngleToGround = CalculateAngle(
      rightShoulderToHipsAngleToGroundMiddlePoint,
      landmarks[24],
      landmarks[12]
    );
    ...
    allToGroundAngles = [
      leftArmAngleToGround,
      rightArmAngleToGround,
      leftForearmAngleToGround,
      rightForearmAngleToGround,
      leftLegAngleToGround,
      rightLegAngleToGround,
      leftShinAngleToGround,
      rightShinAngleToGround,
  ...
  function NormalizeLandmarks(landmarks) {
    ...
    for (let i = 0; i < landmarks.length; i++) {
      normalizedLandmark = {
        x: (landmarks[i].x - minX) / (maxX - minX),
        y: (landmarks[i].y - minY) / (maxY - minY),
        z: (landmarks[i].z - minZ) / (maxZ - minZ),
      };
      normalizedLandmarks.push(normalizedLandmark);
    }
    return [normalizedLandmarks, boundingBox];
  }
  ...
  function CompareLandmarks(landmarks, targetLandmarks) {
    ...
    for (let i = 0; i < landmarks.length; i++) {
      landmarkDifference = {
        x: landmarks[i].x - targetLandmarks[i].x,
        y: landmarks[i].y - targetLandmarks[i].y,
        z: landmarks[i].z - targetLandmarks[i].z,
      };
  ...
  function CalculateTotalDifference(landmarkDifferences) {
    ...
    for (let i = 0; i < landmarkDifferences.length; i++) {
      totalDifference +=
        Math.abs(landmarkDifferences[i].x) +
        Math.abs(landmarkDifferences[i].y) +
        Math.abs(landmarkDifferences[i].z);
    }
